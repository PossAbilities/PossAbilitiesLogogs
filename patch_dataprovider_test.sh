...
const mockGetPublicUrl = vi.fn((path) => ({ data: { publicUrl: `mock-url-${path}` } }));
...
const TestConsumer = () => {
  const { news, events, easyReads, fetchNews, fetchEvents, fetchEasyReads } = useData();
  return (
    <div>
      <div data-testid="news-count">{news.length}</div>
      <div data-testid="events-count">{events.length}</div>
      <div data-testid="easyreads-count">{easyReads.length}</div>
      <div data-testid="news-title">{news[0]?.title}</div>
      <div data-testid="events-title">{events[0]?.title}</div>
      <div data-testid="easyreads-title">{easyReads[0]?.title}</div>
      <div data-testid="easyreads-visible">{easyReads[0]?.showOnPortal?.toString()}</div>
      ...
      <button onClick={fetchEasyReads} data-testid="fetch-easyreads-btn">Fetch Easy Reads</button>
    </div>
  );
};
...
describe('DataProvider', () => {
  ...
  it('provides context to children and fetches data on mount', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('easyreads-title')).toHaveTextContent('Supabase PDF');
      expect(screen.getByTestId('easyreads-visible')).toHaveTextContent('true');
    });
  });
  ...
  it('handles supabase fetch error gracefully', async () => {
    ...
    await waitFor(() => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining('Using mock easy reads data (Supabase fetch failed):'),
        expect.any(Error)
      );
    });
  });
  ...
  it('manual fetch calls update context', async () => {
    ...
    screen.getByTestId('fetch-easyreads-btn').click();

    await waitFor(() => {
      expect(screen.getByTestId('easyreads-title')).toHaveTextContent('Manual Fetch PDF');
      expect(screen.getByTestId('easyreads-visible')).toHaveTextContent('false');
    });
  });
});
INNER_EOF
