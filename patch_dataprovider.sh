...
interface DataContextType {
  ...
  toggleEasyReadVisibility: (id: string, currentStatus: boolean) => Promise<void>;
}
...
export function DataProvider({ children }: { children: ReactNode }) {
      ...
      if (data && data.length > 0) {
        ...
        const mappedData: EasyReadItem[] = data.map((item: any) => {
          ...
          return {
            ...
            coverUrl,
            fileUrl,
            showOnPortal: item.show_on_portal !== false // Defaults to true if null/undefined
          };
        });
  ...
  const toggleEasyReadVisibility = async (id: string, currentStatus: boolean) => {
    try {
      const { error } = await supabase
        .from('pdfs')
        .update({ show_on_portal: !currentStatus })
        .eq('id', id);
      ...
      setEasyReads((prev) =>
        prev.map((item) =>
          item.id === id ? { ...item, showOnPortal: !currentStatus } : item
        )
      );
    } catch (error) {
      console.error("Error toggling easy read visibility:", error);
      throw error;
    }
  };
  ...
  return (
    <DataContext.Provider value={{
      ...
      fetchNews, fetchEvents, fetchEasyReads, toggleEasyReadVisibility,
